...
//  FirstProject
//
//  Created by Андрей Дитякин on 27.03.2023.
//
...
class PhotoService {
    ...
    func photo(atIndexpath indexPath: IndexPath, byUrl url: String) -> UIImage? {
        ...
        if let photo = images[url] {
            ...
            image = photo } else { loadPhoto(atIndexpath: indexPath, byUrl: url) }
    return image
    }
...
extension PhotoService {
    private class Table: DataReloadable { let table: UITableView
        ...
        func reloadRow(atIndexpath indexPath: IndexPath) { table.reloadRows(at: [indexPath], with: .none)
        }
    }
    private class Collection: DataReloadable { let collection: UICollectionView
        ...
        func reloadRow(atIndexpath indexPath: IndexPath) { collection.reloadItems(at: [indexPath])
        }
    }
}
